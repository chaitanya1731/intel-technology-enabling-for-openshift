...
version = getenv("CURRENT_VERSION", default="unknown")
release = version

# Versions list using tags in the lower left corner
from git import Repo
repo = Repo( search_parent_directories=True )
github_repo = (getenv('GITHUB_REPOSITORY') or "intel-technology-enabling-for-openshift")
release_versions = list()
release_versions.append(("main", ""))

tag_versions = reversed([tag.name for tag in repo.tags])
for tag in tag_versions:
    release_versions.append((tag, tag))
